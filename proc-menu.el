...
;; Copyright (C) 2000  Marco Lombardi

;; $Id: proc-menu.el,v 1.4 2006/01/18 16:27:02 mmc Exp $

;;        Name: Process menu
;;   $Revision: 1.4 $
;;       $Date: 2006/01/18 16:27:02 $
...
;; in your .emacs file.
;;
;; I would like to thank Christoph Conrad for helpful suggestions.
...
(if Process-menu-mode-map
  ...
  (define-key Process-menu-mode-map "h" 'describe-mode)
...
(defun list-processes (&optional ignore) ;for emacs-22 compatibility
	...
	(while pl
	  (let* ((process (car pl))
	    ...
	    (cond
	     ...
	     ((or (eq status 'closed) (eq status 'open))
	      (if (eq status 'closed)
		  (setq ch 'c)
		(setq ch 'o))
	      (setq buffer nil)
