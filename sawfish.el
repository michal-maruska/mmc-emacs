...
;(defconst sawfish-warning-keyword-list nil)
...
;;  words  in the value
;;       + in sawfish-extra-keyword-list
;; sawfish-defines-regexp
;; lisp-font-lock-keywords-2
;; =========================
;; sawfish-additional-keywords

(defun sawfish-font-lock-compute-keywords (symbol value) ; custom setter
  ;;symbol should be 'sawfish-warning-keyword-list
  ""
  (set symbol value)
  (setq
   sawfish-additional-keywords
...
;; (get 'sawfish-warning-keyword-list 'saved-value)
;; (put 'sawfish-warning-keyword-list 'saved-value ())
; (setq sawfish-warning-keyword-list ())
'(defcustom sawfish-warning-keyword-list
   '("fixme:?" "FIXME" "Fixme" "fix me" "Fix me" "!!!" "Grrr" "Bummer" "BUG")
  "List of keywords for Sawfish used in highlighting.
Highlight these expressions with `font-lock-warning-face' even if
already fontified."
  :group 'sawfish
;  :type '(repeat (string :tag "Keyword: "))
  :set (function sawfish-font-lock-compute-keywords)
  )
(defvar sawfish-warning-keyword-list '("fixme:?" "FIXME" "Fixme" "fix me" "Fix me" "!!!" "Grrr" "Bummer" "BUG")
  "List of keywords for Sawfish used in highlighting.
Highlight these expressions with `font-lock-warning-face' even if
already fontified.")
  ...
  (if (and (boundp 'running-xemacs) (symbol-value 'running-xemacs))
    ...
    (unless font-lock-defaults
      (set (make-local-variable 'font-lock-defaults)
           (cdr (assoc 'emacs-lisp-mode font-lock-defaults-alist)))) ; aget ??
...
;; see  comint.el
(defvar sawfish-eval-buffer nil
  "here lives the subprocess, which we use for internal purposes")
...
set by the variable `sawfish-result-buffer'"
  (with-temp-buffer
	...
	(bury-buffer sawfish-result-buffer)))))))
...
(defun sawfish-jump-to-info-documentation (symbol)
...
`current-buffer' must be the info buffer you are searching."
  (prog1
      (search-forward-regexp (format "^ - .*: .* %s" symbol) nil t) ; mmc:  relaxed to recognize C functions
    (beginning-of-line)))
...
(defun sawfish-extract-info-entry (symbol)
  ...
  (when (sawfish-jump-to-info-documentation symbol)
...
(defun sawfish-info-variable (variable)
  ...
  (sawfish-jump-to-info-documentation variable))
...
;; i would like a scheme macro ...
(defun sawfish-complete ()
  "Complete a sf symbol at point."
   (interactive)
   ;; Find the beginning of the string to complete, bind variables
   (let ((beg (save-excursion (skip-syntax-backward "_w") (point))) ;  fixme !! a-zA-Z_-
         (completion-ignore-case t)
         (cib-adaptive-case t)
         (cib-sort-completions t)
         (cib-uniquify-completions t))
     (cib-complete beg 'sawfish-complete-internal)))

(define-key sawfish-mode-map [(meta control ?i)] 'sawfish-complete)


;;  in case of error ???
(defun sawfish-complete-internal (start predicate op)
  ""
  (let ((sym-re (concat "^" (regexp-quote (erase-text-properties start))))
	completions)
    (cond ((eq op 't)			;all-
	   (setq completions
		 (sawfish-eval-read
...
(set-keymap-parent sawfish-mode-map emacs-lisp-mode-map)
